...
public class TempRangeSeekBar<T extends Number> extends ImageView {
    /**
     * Default color of a {@link TempRangeSeekBar}, #FF33B5E5. This is also known as "Ice Cream Sandwich" blue.
     */
    public static final int CRIT_COLOR = Color.parseColor("#FE2E2E");
    public static final int WARN_COLOR = Color.parseColor("#FFA500");
    public static final int NORM_COLOR = Color.parseColor("#31B404");
    ...
    public static Integer DEFAULT_WARNING = 0;
    public static Integer DEFAULT_CRITICAL = 50;
    ...
    private T absoluteWarningValue, absoluteCriticalValue;
    private NumberType numberType;
    private double absoluteWarningValuePrim, absoluteCriticalValuePrim;
    private double normalizedWarningValue = 0d;
    private double normalizedCriticalValue = 1d;
    ...
    public TempRangeSeekBar(Context context) {
        super(context);
        init(context, null);
    }

    public TempRangeSeekBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public TempRangeSeekBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs == null) {
            ...
            mNormalColor = NORM_COLOR;
            mWarningColor = WARN_COLOR;
            mCriticalColor = CRIT_COLOR;
            ...
            try {
                setRangeValues(
                        extractNumericValueFromAttributes(a, R.styleable.RangeSeekBar_absoluteWarningValue, DEFAULT_WARNING),
                        extractNumericValueFromAttributes(a, R.styleable.RangeSeekBar_absoluteCriticalValue, DEFAULT_CRITICAL)
                );
                mShowTextAboveThumbs = a.getBoolean(R.styleable.RangeSeekBar_valuesAboveThumbs, true);
                //change text color
                mTextAboveThumbsColor = a.getColor(R.styleable.RangeSeekBar_textAboveThumbsColor, Color.parseColor("#5A84BB"));
                ...
                mWarningColor = a.getColor(R.styleable.RangeSeekBar_warningColor, WARN_COLOR);
                mNormalColor = a.getColor(R.styleable.RangeSeekBar_normalColor, NORM_COLOR);
                mCriticalColor = a.getColor(R.styleable.RangeSeekBar_criticalColor, CRIT_COLOR);
    ...
    public void setRangeValues(T warningValue, T criticalValue) {
        this.absoluteWarningValue = warningValue;
        this.absoluteCriticalValue = criticalValue;
        setValuePrimAndNumberType();
    }
    ...
    private void setRangeToDefaultValues() {
        this.absoluteWarningValue = (T) DEFAULT_WARNING;
        this.absoluteCriticalValue = (T) DEFAULT_CRITICAL;
        setValuePrimAndNumberType();
    }

    private void setValuePrimAndNumberType() {
        absoluteWarningValuePrim = absoluteWarningValue.doubleValue();
        absoluteCriticalValuePrim = absoluteCriticalValue.doubleValue();
        numberType = NumberType.fromNumber(absoluteWarningValue);
    }

    @SuppressWarnings("unused")
    public void resetSelectedValues() {
        setSelectedWarningValue(absoluteWarningValue);
        setSelectedCriticalValue(absoluteCriticalValue);
    }
    ...
    /**
     * Returns the absolute warning value of the range that has been set at construction time.
     *
     * @return The absolute warning value of the range.
     */
    ...
    /**
     * Returns the absolute critical value of the range that has been set at construction time.
     *
     * @return The absolute critical value of the range.
     */
    ...
    /**
     * Returns the currently selected warning value.
     *
     * @return The currently selected warning value.
     */
    ...
    /**
     * Sets the currently selected warning value. The widget will be invalidated and redrawn.
     *
     * @param value The Number value to set the warning value to. Will be clamped to given absolute warning/critical range.
     */
    public void setSelectedWarningValue(T value) {
        // in case absoluteWarningValue == absoluteCriticalValue, avoid division by zero when normalizing.
        if (0 == (absoluteCriticalValuePrim - absoluteWarningValuePrim)) {
            setNormalizedWarningValue(0d);
        } else {
            setNormalizedWarningValue(valueToNormalized(value));
        }
    }

    /**
     * Returns the currently selected critical value.
     *
     * @return The currently selected critical value.
     */
    ...
    /**
     * Sets the currently selected critical value. The widget will be invalidated and redrawn.
     *
     * @param value The Number value to set the critical value to. Will be clamped to given absolute warning/critical range.
     */
    public void setSelectedCriticalValue(T value) {
        // in case absoluteWarningValue == absoluteCriticalValue, avoid division by zero when normalizing.
        if (0 == (absoluteCriticalValuePrim - absoluteWarningValuePrim)) {
            setNormalizedCriticalValue(1d);
        } else {
            setNormalizedCriticalValue(valueToNormalized(value));
        }
    }
            ...
            case MotionEvent.ACTION_MOVE:
                if (pressedThumb != null) {
                    ...
                    if (notifyWhileDragging && listener != null) {
                        listener.onRangeSeekBarValuesChanged(this, getSelectedWarningValue(), getSelectedCriticalValue());
                    }
                }
                break;
    ...
    private void trackTouchEvent(MotionEvent event) {
        ...
        if (Thumb.WARN.equals(pressedThumb) && !mSingleThumb) {
            setNormalizedWarningValue(screenToNormalized(x));
        } else if (Thumb.CRIT.equals(pressedThumb)) {
            setNormalizedCriticalValue(screenToNormalized(x));
        }
    }
...
//        }
    ...
    /**
     * Overridden to save instance state when device orientation changes. This method is called automatically if you assign an id to the RangeSeekBar widget using the {@link #setId(int)} method. Other members of this class than the normalized warning and critical values don't need to be saved.
     */
    @Override
    protected Parcelable onSaveInstanceState() {
        final Bundle bundle = new Bundle();
        bundle.putParcelable("SUPER", super.onSaveInstanceState());
        bundle.putDouble("WARN", normalizedWarningValue);
        bundle.putDouble("CRIT", normalizedCriticalValue);
        return bundle;
    }
    ...
    protected void onRestoreInstanceState(Parcelable parcel) {
        ...
        normalizedWarningValue = bundle.getDouble("WARN");
        normalizedCriticalValue = bundle.getDouble("CRIT");
    }
    ...
    private Thumb evalPressedThumb(float touchX) {
        Thumb result = null;
        boolean warnThumbPressed = isInThumbRange(touchX, normalizedWarningValue);
        boolean critThumbPressed = isInThumbRange(touchX, normalizedCriticalValue);
        if (warnThumbPressed && critThumbPressed) {
            // if both thumbs are pressed (they lie on top of each other), choose the one with more room to drag. this avoids "stalling" the thumbs in a corner, not being able to drag them apart anymore.
            result = (touchX / getWidth() > 0.5f) ? Thumb.WARN : Thumb.CRIT;
        } else if (warnThumbPressed) {
            result = Thumb.WARN;
        } else if (critThumbPressed) {
            result = Thumb.CRIT;
        }
        return result;
    }
    ...
    /**
     * Sets normalized warning value to value so that 0 <= value <= normalized critical value <= 1. The View will get invalidated when calling this method.
     *
     * @param value The new normalized warning value to set.
     */
    private void setNormalizedWarningValue(double value) {
        normalizedWarningValue = Math.max(0d, Math.min(1d, Math.min(value, normalizedCriticalValue)));
        invalidate();
    }

    /**
     * Sets normalized critical value to value so that 0 <= normalized warning value <= value <= 1. The View will get invalidated when calling this method.
     *
     * @param value The new normalized critical value to set.
     */
    private void setNormalizedCriticalValue(double value) {
        normalizedCriticalValue = Math.max(0d, Math.min(1d, Math.max(value, normalizedWarningValue)));
        invalidate();
    }

    /**
     * Converts a normalized value to a Number object in the value space between absolute warning and critical.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Thumb constants (warning and critical).
     */
    private enum Thumb {
        WARN, CRIT
    }
    ...
    public interface OnRangeSeekBarChangeListener<T> {
        ...
        void onRangeSeekBarValuesChanged(TempRangeSeekBar<?> bar, T warnValue, T critValue);
    }
